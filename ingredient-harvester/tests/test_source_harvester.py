...
def test_harvester_prefers_direct_product_url(monkeypatch) -> None:
    from app.harvester import fetch as fetch_mod
    from app.harvester import source_harvester as sh_mod
    ...
    h = SourceHarvester(search_engine=FakeSearch(urls=[]))
    out = h.process(market="US", brand="Test", product_name="P1", preferred_url="https://example.com/direct")
    assert out.status == "OK"
    assert out.source_ref == "https://example.com/direct"


def test_harvester_dedupes_preferred_url_against_search(monkeypatch) -> None:
    from app.harvester import fetch as fetch_mod
    from app.harvester import source_harvester as sh_mod
    ...
    h = SourceHarvester(search_engine=FakeSearch(urls=["https://example.com/direct", "https://example.com/fallback"]))
    out = h.process(market="US", brand="Test", product_name="P1", preferred_url="example.com/direct")
    assert called.count("https://example.com/direct") == 1
    assert out.status == "OK"
    assert out.source_ref == "https://example.com/fallback"
