...
async function proxyJson(req: any, res: any, upstreamPath: string) {
  const base = upstreamBaseUrl();
  const url = `${base}${upstreamPath.startsWith("/") ? "" : "/"}${upstreamPath}`;
  ...
  try {
    ...
    console.error("[/api/parser] proxy error", err);
...
parserRouter.post("/re-parse", async (req, res) => proxyJson(req, res, "/v1/parser/re-parse"));
parserRouter.post("/re-parse-batch", async (req, res) => proxyJson(req, res, "/v1/parser/re-parse-batch"));
