...
export async function extractCatalogV2(input: ExtractV2RequestBody): Promise<ExtractV2Response> {
  ...
  const requestedMarkets = input.markets?.length ? input.markets : [normalizeMarketId(input.market)];
  const profiles = getMarketProfiles(requestedMarkets);
  log("info", `V2 extraction start: ${requestedTarget.domain} | markets=${profiles.map((p) => p.market_id).join(",")}`);

  const primaryResolution = await resolveStorefrontTarget({
    target: requestedTarget,
    marketId: profiles[0]?.market_id || normalizeMarketId(input.market),
    context: buildRequestContext(profiles[0] || getMarketProfiles(["US"])[0]!),
    diagnostics,
    log,
  });
  const target = primaryResolution.target;
  ...
  for (const profile of profiles) {
    ...
    try {
      const result = await extractSingleMarket({
        ...
        maxProductsTotal,
        diagnostics,
        selectorRootDetected: primaryResolution.selectorRootDetected && !primaryResolution.storefrontResolved,
        log,
      });
...
async function extractSingleMarket(params: {
  ...
  const discovered = await discoverProductUrlsShared({
    ...
    context: params.context,
    diagnostics: params.diagnostics!,
    selectorRootDetected: params.selectorRootDetected,
    log: params.log,
  });
  ...
  const browserRun = await runBrowserTaskWithFallback(
    async (browser) =>
      withTimeoutShared(
        mapWithConcurrencyShared(batchCandidates, concurrency, async (url) =>
          scrapeProductPageV2({
            browser,
            url,
            baseUrl: params.baseUrl,
  ...
  const offers = browserRun.result.flatMap((batch) => batch).slice(0, params.batchLimit * 20);
  const nextOffset = params.batchOffset + params.batchLimit;
  ...
  if (offers.length === 0 && params.diagnostics && !params.diagnostics.failure_category) {
    if (params.diagnostics.block_provider) {
      setFailureCategory(params.diagnostics, "bot_challenge");
    } else {
      setFailureCategory(params.diagnostics, "product_schema_missing");
    }
  }
...
async function tryExtractShopifyOffersV2(params: {
  ...
  const probe = await fetchJsonTracked<ShopifyProductsResponse>(probeUrl, params.context, params.diagnostics!);
  if (!probe.data || !Array.isArray(probe.data.products)) {
    return null;
  }

  const pageLimit = clampIntShared(process.env.SHOPIFY_MAX_PAGES, 20, 1, 200);
...
async function scrapeProductPageV2(params: {
  ...
  try {
    ...
    if (params.diagnostics?.block_provider) {
      throw new BotChallengeError(params.diagnostics.block_provider, params.url);
    }
    return [];
  } finally {
...
async function fetchCurrencyHint(
  baseUrl: string,
  context: RequestContext,
  diagnostics: ExtractV2Response["diagnostics"],
): Promise<string | null> {
  const html = await fetchTextTracked(baseUrl, context, diagnostics!);
  ...
  for (const pattern of regexes) {
    const matched = html.body.match(pattern)?.[1];
