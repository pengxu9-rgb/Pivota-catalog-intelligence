...
export function looksLikeFullIngredientListText(text: string | undefined) {
  ...
  return (
    ...
    /\b(?:water\/aqua|aqua\/water)\b/i.test(normalized) ||
...
export async function enrichDirectShopifyPdpResponse(params: {
  ...
  const htmlSnapshot = await htmlFetcher(params.seedUrl, params.diagnostics);
  if (htmlSnapshot.body) {
    ...
    if (htmlProduct) {
      ...
      if (!htmlStillMissingImages && !htmlStillMissingPdpFields) {
        params.log("success", `Recovered Shopify PDP fields from native HTML snapshot: ${params.seedUrl}`);
        return htmlMerged;
      }

      if (!productMissingImages && !variantMissingImages && !htmlStillMissingPdpFields) {
        params.log("success", `Recovered Shopify PDP modules from native HTML snapshot: ${params.seedUrl}`);
        return htmlMerged;
      }
    }
  }
